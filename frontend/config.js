...
<div>
  ...
  <div class="container container-narrow">
    ...
    <div class="cfg-card">
      ...
      <label class="cfg-label"><span class="cfg-label-text">Model 1 (best value)</span>
      ...
      <h4 style="margin-bottom: 8px;">Model 2 (quality tier — optional)</h4>
      ...
      <p class="cfg-hint">Leave Model 2 URL/Key empty to share Model 1's relay. Fill them to use a different provider/key.</p>
      <label class="cfg-label">
        <span class="cfg-label-text">Model 2 API key (optional)</span>
        <span class="status-badge" :class="cfg.fallback_2_api_key_set ? 'st-set' : 'st-unset'">{{ cfg.fallback_2_api_key_set ? 'Set ✓' : 'Uses Model 1 key' }}</span>
      </label>
      <input type="password" v-model="cfg.fallback_2_api_key" placeholder="Leave empty to use Model 1 key"
             class="cfg-input long" autocomplete="off">
      <label class="cfg-label"><span class="cfg-label-text">Model 2 base URL (optional)</span>
        <span class="status-badge" :class="cfg.fallback_2_base_url ? 'st-set' : 'st-unset'">{{ cfg.fallback_2_base_url ? 'Set ✓' : 'Uses Model 1 URL' }}</span>
      </label>
      <input type="text" v-model="cfg.fallback_2_base_url" class="cfg-input long" placeholder="https://openrouter.ai/api/v1">
      <p class="cfg-hint">On save, Nyaa pings each configured URL with its key. If Model 2 shares Model 1's URL/key, it's validated once. If different, both are validated separately.</p>
    </div>
